...
var (
	...
	// client is the get3w client being tested.
	client *Client
...
// setup sets up a test HTTP server along with a get3w.Client that is
...
func setup() {
	...
	// get3w client configured to use test server
	client = NewClient("")
...
func openTestFile(name, content string) (file *os.File, dir string, err error) {
	dir, err = ioutil.TempDir("", "go-get3w")
	if err != nil {
		return nil, dir, err
	}
...
func TestNewRequest(t *testing.T) {
	...
	inBody, outBody := &User{Username: "u"}, `{"username":"u"}`+"\n"
...
// If a nil body is passed to get3w.NewRequest, make sure that nil is also
...
func TestResponse_populatePageValues(t *testing.T) {
	r := http.Response{
		Header: http.Header{
			"Link": {`<https://api.get3w.com/?page=1>; rel="first",` +
				` <https://api.get3w.com/?page=2>; rel="prev",` +
				` <https://api.get3w.com/?page=4>; rel="next",` +
				` <https://api.get3w.com/?page=5>; rel="last"`,
			},
		},
	}
...
func TestResponse_populatePageValues_invalid(t *testing.T) {
	r := http.Response{
		Header: http.Header{
			"Link": {`<https://api.get3w.com/?page=1>,` +
				`<https://api.get3w.com/?page=abc>; rel="first",` +
				`https://api.get3w.com/?page=2; rel="prev",` +
				`<https://api.get3w.com/>; rel="next",` +
				`<https://api.get3w.com/?page=>; rel="last"`,
			},
		},
	}
	...
	r = http.Response{
		Header: http.Header{
			"Link": {`<https://api.get3w.com/%?page=2>; rel="first"`},
		},
	}
...
// function.  A redirect loop is pretty unlikely to occur within the get3w
